...
const getEventById = async(id)=>{
    const event = await Event.findOne({_id:id,isDeleted:false,isAdmin:true});
    if(!event){
...
const createEvent = async (eventBody,user) => {
  ...
  try{
    ...
    const { name, startDate, endDate, location, categoryId,latitude,longitude } = eventBody;
    const userId = user._id;

    const [event] = await Event.create([{ name, startDate, endDate, location,latitude,longitude,categoryId, userId,isAdmin:true }], { session });
...
const deleteEvent = async(eventId)=>{
  ...
  try {
    const event = await Event.findOne({_id:eventId,isDeleted:false,isAdmin:true}).session(session);
...
const editEvent = async (eventBody, user) => {
    ...
    try {
      const { eventId, name, startDate, endDate, location, categoryId,latitude,longitude } = eventBody;
      const userId = user._id;
  
      const event = await Event.findOne({ _id: eventId, userId, isDeleted: false, isAdmin: true }).session(session);
...
const listEvents = async(reqBody)=>{
    const {page=1,limit=10,categoryId,search,sortBy="createdAt",sortOrder=-1} = reqBody;
    let sortValue={};
